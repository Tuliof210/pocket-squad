...
fabricate a default to avoid the question. Only once the goal is clear, generate
`.squad/stories/<YYYY-MM-DD>-<slug>/` folders — one per Story — each with `story.md`
(status: draft), `tasks/` and `board.md`. Large requests split into multiple Stories
(one PR each), ordered via story-level `depends_on`.
